...
export default function NotFound() {
  ...
  return (
    <section className="min-h-screen bg-[var(--background)] text-[var(--foreground)] flex items-center justify-center relative overflow-hidden px-6 font-mono selection:bg-[var(--accent)]/30 selection:text-white">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(to_bottom,rgba(255,255,255,0),rgba(255,255,255,0)_50%,rgba(0,0,0,0.3)_50%,rgba(0,0,0,0.3))] bg-[size:100%_4px] pointer-events-none z-[var(--z-base)] opacity-40" />

      {/* Cybernetic Aura Ambient Glow */}
      <div className="absolute w-[600px] h-[600px] rounded-full bg-[var(--accent)]/5 blur-[120px] pointer-events-none -top-40 -left-40 animate-pulse" style={{ animationDuration: '7s' }} />
      <div className="absolute w-[500px] h-[500px] rounded-full bg-[var(--accent)]/5 blur-[100px] pointer-events-none -bottom-40 -right-40 animate-pulse" style={{ animationDuration: '9s' }} />

      <div className="relative z-[var(--z-card)] max-w-3xl w-full flex flex-col items-center space-y-10 py-12 md:py-20">
        
        {/* Flag Indicator */}
        <div className="flex items-center gap-3 px-6 py-2.5 rounded-full border border-[var(--accent)]/25 bg-[var(--accent)]/5 text-[var(--accent)] text-[10px] uppercase tracking-widest animate-pulse">
        ...
        <div className="w-full bg-[var(--surface)]/95 border border-[var(--border)] rounded-3xl shadow-[0_0_60px_rgba(168,85,247,0.1)] overflow-hidden flex flex-col min-h-[420px] md:min-h-[480px] backdrop-blur-md">
          
          {/* Terminal Window Header Bar */}
          <div className="px-6 py-4.5 bg-[var(--surface-2)] border-b border-[var(--border)] flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-[var(--accent)]/80 font-bold">
          ...
          <form 
            ...
            className="p-6 md:p-8 bg-[var(--background)] border-t border-[var(--border)] flex items-center gap-4"
          >
            <span className="text-[var(--accent)] font-bold text-xs md:text-sm shrink-0 select-none">guest@salahuddin.codes:~$</span>
            <input
              type="text"
              autoFocus
              value={input}
              ...
              className="flex-1 bg-transparent text-white font-mono text-xs md:text-sm placeholder-[var(--muted-soft)]/40 select-all !border-0 !outline-none !ring-0 !shadow-none !p-0 !m-0"
              style={{
                border: 'none',
                outline: 'none',
                boxShadow: 'none',
            ...
            <button 
              type="submit" 
              className="text-[var(--accent)] hover:text-white p-2 rounded-lg bg-[var(--accent)]/10 hover:bg-[var(--accent)]/25 transition-all shrink-0 cursor-pointer"
            >
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          <span className="text-[10px] uppercase tracking-widest text-[var(--accent)]/70 font-bold">Quick macros:</span>
          {["home", "about", "projects", "contact", "matrix", "ping"].map((macro) => (
            <button
              key={macro}
              onClick={() => handleCommand(macro)}
              className="px-4 py-2 text-[10px] font-mono uppercase tracking-wider rounded-xl border border-[var(--border)] bg-[var(--surface)] hover:bg-[var(--accent)] hover:text-white hover:border-[var(--accent)] transition-all duration-300 shadow-md"
            >
              Run: {macro}
            </button>
          ))}
